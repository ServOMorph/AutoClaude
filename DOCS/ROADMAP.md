...
> Statut global : **Phase 0 — Démarrage** (v1 stable, v2.0 non démarrée)
...
| 1.3 Déplacer assets dans `assets/` | `assets/yes.png`, `assets/logo.png` | ⏳ | Adapter les imports après déplacement |
| 1.4 `src/config/constants.py` | `src/config/constants.py` | ⏳ | URLs, chemins assets, couleurs hex, version |
| 1.5 `src/config/settings.py` | `src/config/settings.py` | ⏳ | Persistance JSON dans `~/.autoclaude/settings.json` |
| 1.6 Mettre à jour `requirements.txt` | `requirements.txt` | ⏳ | Ajouter `customtkinter>=5.2.2`, `Pillow>=10.0.0` |
...
| Tâche | Fichier cible | Statut | Notes |
|-------|--------------|--------|-------|
| 2.1 `src/core/monitors.py` | `src/core/monitors.py` | ⏳ | Extraire `get_all_monitors()` de `run.py:87-102` |
| 2.2 `src/core/detector.py` | `src/core/detector.py` | ⏳ | Extraire la chaîne de détection `run.py:104-193` |
| 2.3 `src/core/clicker.py` | `src/core/clicker.py` | ⏳ | Extraire `click_with_project()` + `click_with_pyautogui()` de `run.py:195-217` |
| 2.4 `src/core/listener.py` | `src/core/listener.py` | ⏳ | Extraire `on_press()` + `on_mouse_move()` de `run.py:219-243` |
| 2.5 `src/core/autoclick_service.py` | `src/core/autoclick_service.py` | ⏳ | Orchestrateur : boucle `main()` → classe `AutoclickService` avec threading |
| 2.6 Vérification comportement identique v1 | — | ⏳ | Test manuel : détection + clic + Esc + `--auto-stop` |
...
| Tâche | Fichier cible | Statut | Notes |
|-------|--------------|--------|-------|
| 3.1 `src/security/claude_md_protector.py` | `src/security/claude_md_protector.py` | ⏳ | Création `.claude/CLAUDE.md`, injection bloc AUTOCLAUDE_GUARD |
| 3.2 Méthode `apply()` | — | ⏳ | Crée/modifie le fichier, retourne `(bool, str)` |
| 3.3 Méthode `is_already_protected()` | — | ⏳ | Vérifie présence des marqueurs START/END |
| 3.4 Méthode `remove_protection()` | — | ⏳ | Supprime le bloc entre marqueurs |
| 3.5 Test manuel | — | ⏳ | Vérifier sur un dossier projet test |
...
| 4.3 Définition polices (Space Grotesk / fallback Segoe UI) | — | ⏳ | `CTkFont` avec fallback automatique |
...
| Tâche | Fichier cible | Statut | Notes |
|-------|--------------|--------|-------|
| 5.1 `src/ui/components/header.py` | `src/ui/components/header.py` | ⏳ | Logo 80px + titre + lien serenia-tech.fr + slogan |
| 5.2 `src/ui/components/warning_banner.py` | `src/ui/components/warning_banner.py` | ⏳ | Carte orange 15% opacité, bordure gauche 4px |
| 5.3 `src/ui/components/activate_button.py` | `src/ui/components/activate_button.py` | ⏳ | Toggle ON/OFF, pill-shaped, dot indicateur |
| 5.4 `src/ui/components/protection_button.py` | `src/ui/components/protection_button.py` | ⏳ | Style secondary, appelle `ClaudeMdProtector` |
| 5.5 `src/ui/components/footer.py` | `src/ui/components/footer.py` | ⏳ | Liens site + GitHub + version |
...
| Tâche | Fichier cible | Statut | Notes |
|-------|--------------|--------|-------|
| 6.1 `src/ui/app.py` | `src/ui/app.py` | ⏳ | Classe `AutoClaudeApp(ctk.CTk)`, 520×640px, non redimensionnable |
| 6.2 Assemblage des composants | — | ⏳ | Header, warning, activate, protection, footer |
| 6.3 Binding Esc → stop service | — | ⏳ | Conserver comportement v1 |
...
| Tâche | Fichier cible | Statut | Notes |
|-------|--------------|--------|-------|
| 7.1 Réécrire `run.py` (≤10 lignes) | `run.py` | ⏳ | Import `AutoClaudeApp`, `app.mainloop()` |
| 7.2 Vérification lancement `python run.py` < 2s | — | ⏳ | Critère de validation |
...
| Liens `serenia-tech.fr` et `github.com/ServOMorph` s'ouvrent | ⏳ |
